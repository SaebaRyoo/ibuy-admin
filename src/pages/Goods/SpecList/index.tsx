...
import { specList, addSpec, editSpec, delSpec } from '@/services/aitao/goods/spec';
...
const Goods: React.FC = () => {
            ...
            modal.confirm({
              ...
              onOk: async () => {
                handleModalOperation(
                  async () => await delSpec({ id: record.id }),
  ...
  const handleConfirm = (values: any, openType: string) => {
    ...
    const request =
      openType === Add ? async () => await addSpec(values) : async () => await editSpec(values);
    ...
    <PageContainer>
      <ProTable<API.Spec, API.PageParams>
        ...
        request={async (params, sort, filter) => {
          const { data } = await specList(params);
          return {
            data: data.list || [],
            // success 请返回 true，
